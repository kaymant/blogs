// --- Global Variables and Setup ---
        ...
        function drawGrid(A, t, v, eigenvectors) { 
            ...
            if (isSingular) {
              const epsilon = 1e-6; // Use the same small tolerance
              const isZeroMatrix = 
              Math.abs(A[0][0]) < epsilon && 
              Math.abs(A[0][1]) < epsilon && 
              Math.abs(A[1][0]) < epsilon && 
              Math.abs(A[1][1]) < epsilon;

              ctx.fillStyle = 'red';
              ctx.font = '20px sans-serif';
    
             if (isZeroMatrix) {
                     ctx.fillText('WARNING: Zero Matrix. Space collapses to a point.', 10, 30);
             } else {
